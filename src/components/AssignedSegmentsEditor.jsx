...
// builds a log — one target time per segment (applies to each rep), not
// per individual rep, since these are targets rather than actuals.
export default function AssignedSegmentsEditor({ segments, onChange }) {
  ...
  function addSegment() {
    onChange([...segments, emptyAssignedSegment()])
  }
          ...
          return (
            <div className="segment-editor-card" key={seg.key}>
              ...
              <div className="form-row">
                ...
                <span className="segment-rep-pace">{targetPace || ''}</span>
              </div>
            </div>
          )
        })}
