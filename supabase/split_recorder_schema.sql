...
-- splits log for a given day.
--
-- p_rep_times: jsonb array of {"hours":n,"minutes":n,"seconds":n}, one per
-- filled-in column, already trimmed client-side to only the columns the
-- coach actually entered a time for (an athlete with 3 of 4 columns filled
-- gets a 3-element array here, not a 4-element one with a gap).
--
...
begin
  ...
  if p_distance_value is null or p_distance_value <= 0 then
    raise exception 'A positive distance is required';
  end if;
  ...
  v_rep_count := coalesce(jsonb_array_length(p_rep_times), 0);
  ...
  execute format(
    'insert into public.%I (workout_id, order_index, label, distance_value, distance_unit, reps) values ($1, 0, null, $2, $3, $4) returning id',
    v_segments_table
  ) into v_segment_id using v_workout_id, p_distance_value, p_distance_unit, v_rep_count;

  for v_rep in select * from jsonb_array_elements(p_rep_times)
  loop
    v_idx := v_idx + 1;
    execute format(
      'insert into public.%I (segment_id, rep_number, time_hours, time_minutes, time_seconds) values ($1, $2, $3, $4, $5)',
      v_reps_table
    ) using v_segment_id, v_idx,
...
grant execute on function public.record_split_recorder_entry(uuid, date, text, text, numeric, text, uuid, jsonb) to authenticated;
