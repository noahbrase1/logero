...
-- — it empties every application table. Run the verification query at the
-- bottom afterward and confirm every count is 0 before doing anything else.
--
-- This does NOT touch auth.users (Supabase's own auth schema) — delete
-- those separately via the Dashboard (Authentication → Users) AFTER running
-- this script. See the note at the bottom for why this order matters.
